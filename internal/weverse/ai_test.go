...
func TestAIConversationLifecycle(t *testing.T) {
	...
	for i, id := range []string{"one", "two", "two"} {
		e := Event{ID: id, Kind: "comment", MemberID: "member", CommunityID: 235, Author: "IAN", Body: "reply", ParentBody: "context", ParentAuthor: "fan", Time: now.Add(time.Duration(i) * time.Second).UnixMilli()}
		if err := CollectAI(dir, sub, e, now); err != nil {
			t.Fatal(err)
		}
	}
	_ = CollectAI(dir, sub, Event{ID: "other", Kind: "comment", MemberID: "other", CommunityID: 235, Time: now.UnixMilli()}, now.Add(4*time.Minute))
...
func TestAIUsesWholeOriginalConversationAndSanitizesErrors(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if payload.Model != "grok-4.5" || len(payload.Messages) != 2 || !strings.Contains(payload.Messages[1].Content, "parent-original") || !strings.Contains(payload.Messages[1].Content, "second-reply") || !strings.Contains(payload.Messages[1].Content, "artist") || !strings.Contains(payload.Messages[1].Content, "完整通顺") {
			t.Error("missing context")
		}
		w.WriteHeader(code)
		if code == 200 {
			content, _ := json.Marshal(map[string]any{"translations": []map[string]string{{"id": "one", "parentChinese": "上下文", "replyChinese": "第一条"}, {"id": "two", "parentChinese": "", "replyChinese": "第二条"}}, "summary": "完整上下文翻译"})
	...
	b := AIBatch{Author: "IAN", Entries: []AIEntry{{ID: "one", Author: "IAN", ParentAuthor: "粉丝", ParentBody: "parent-original", Body: "first-reply"}, {ID: "two", Author: "IAN", Body: "second-reply"}}}
...
func TestAISeparatesFansAndMergesMemberThreads(t *testing.T) {
	dir := t.TempDir()
	...
	if len(state.Batches) != 3 {
		t.Fatal("fans incorrectly merged", len(state.Batches))
	}
	pair := state.Batches["sub/thread:p"]
	if pair == nil || len(pair.Entries) != 2 || len(pair.ActorIDs()) != 2 || pair.Entries[1].AuthorMemberID != "b" {
		t.Fatal("member interaction not merged", pair)
	}
	if len(state.Batches["sub/a"].Entries) != 2 || len(state.Batches["sub/b"].Entries) != 1 {
		t.Fatal("member fan sessions contaminated")
	}
	if job, _ := ClaimAIJob(dir, cfg, ai, start.Add(6*time.Minute), true); job != nil {
		t.Fatal("member still active but summary sent")
	}
	job, err := ClaimAIJob(dir, cfg, ai, start.Add(7*time.Minute), true)
	if err != nil || job == nil || job.MemberID != "b" || len(job.Entries) != 1 {
		t.Fatal("wrong fan conversation completed", err)
	}
	_ = Read(dir, "ai-state.json", &state)
	if state.Batches["sub/thread:p"] == nil {
		t.Fatal("interaction closed before all members stopped")
	}
