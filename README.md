...
内置多国随机地址生成器（`bot addr gen <国家代码>`），可生成包含真实姓名、街道、城市、邮编、电话的完整身份信息。
...
**注意**：所有数据从预定义池中随机组合（非实时 API 查询），池子较大（每个国家数百条目）但本质上是离线生成——不会生成现实中不存在的城市名，但生成的地址组合（城市+街道+号码）不保证现实中存在该具体地址。

**输出示例：**
```
┌─ Address ─────────────────────────────┐
  First Name: Daniel                Last Name: Cooper
  Country:    United States
  State/Prov: AZ                   City:       Phoenix
  Address:    3238 Eastwood Dr
  Post Code:  07246
  Phone:      +1 (328) 150-3935
└────────────────────────────────────────┘
```
