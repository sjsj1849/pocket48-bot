...
type Result = { feedProbe?:{pending:boolean;success:boolean;username?:string;error?:string}; requestState?: {requestsLastHour:number;nextRetryAt?:string;reason?:string}; browser?: {configured:boolean;pending:boolean;updatedAt?:number;error?:string}; settings: Settings; sessionConfigured: boolean; sessionUsername?: string; status: { lastCheck?: string; lastSuccess?: string; error?: string; targets?: Record<string,string> } }
...
export function InstagramConfig({ defaultGroup }: { defaultGroup: string }) {
 ...
 return <div className="platform-config x-config">
  ...
  <section className="platform-section"><h3>登录与运行</h3>
   ...
   <p className="muted">内置浏览器：{data?.browser?.configured ? '已保存 Instagram Cookie' : '尚未导入'}{data?.browser?.pending ? ' · 更新待验证，保留采集器现有会话' : ''}。浏览器更新 Cookie 后自动同步，验证成功再替换采集登录态；保存到浏览器不保证永不过期。</p>
   <div className="inline-actions"><button className="secondary-button" disabled={!!busy||!data?.sessionConfigured} onClick={()=>void action('browser-import',async()=>{const r=await api<{message:string}>('instagram/browser',{method:'POST',body:JSON.stringify({action:'import'})});setData(await api<Result>('instagram/settings'));setMessage(r.message)})}>导入到内置浏览器</button><button className="secondary-button" disabled={!!busy} onClick={()=>void action('browser-sync',async()=>{const r=await api<{message:string}>('instagram/browser',{method:'POST',body:JSON.stringify({action:'sync'})});setData(await api<Result>('instagram/settings'));setMessage(r.message)})}>同步浏览器登录态</button><button className="secondary-button" disabled={!!busy} onClick={()=>void action('browser-open',async()=>{await api('instagram/browser',{method:'POST',body:JSON.stringify({action:'open'})});window.dispatchEvent(new CustomEvent('pocket48-navigate',{detail:'browser'}))})}>打开 Instagram 浏览器</button><button className="secondary-button" disabled={!!busy} onClick={()=>void action('refresh',async()=>setData(await api<Result>('instagram/settings')))}>刷新状态</button></div>
   {data?.browser?.error&&<p className="inline-error">{data.browser.error}</p>}
