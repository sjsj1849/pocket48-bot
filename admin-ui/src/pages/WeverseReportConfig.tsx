...
export function WeverseReportConfig() {
  ...
  const [weekDate, setWeekDate] = useState(() => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date()))
  ...
  useEffect(() => { setReport(undefined) }, [kind, year, month, weekDate])
  const query = kind === 'weekly' ? `kind=weekly&date=${encodeURIComponent(weekDate)}` : `kind=${kind}&year=${year}&month=${month}`
  ...
  return <section className="platform-section">
    ...
    {settings && <>
      ...
      <div>
        <PlatformToggle label="周报：每周一（统计上周一至周日）" checked={settings.weekly} onChange={checked => setSettings({ ...settings, weekly: checked })} />
      ...
      <p className="muted">报表沿用微博日报的发件与收件配置。发送前自动回采当期可见内容；失败后重试，成功后不重复发送。首次启用不会自动补发早期报告。周报以周一至周日为一周，选择周内任意日期即可预览或发送该周报告。未结束的周或月份是截至已采集时刻的预览。</p>
      <div className="wv-fields">
        <label>报表类型<select disabled={!!busy} value={kind} onChange={e => setKind(e.target.value)}><option value="weekly">周报</option><option value="monthly">月报</option><option value="firstHalf">上半年报</option><option value="annual">年报</option></select></label>
        {kind !== 'weekly' && <label>报表年份<input type="number" disabled={!!busy} min="2025" max="2100" value={year} onChange={e => setYear(Number(e.target.value))} /></label>}
        {kind === 'weekly' && <label>周内日期（北京时间）<input type="date" disabled={!!busy} min="2025-01-01" max="2100-12-31" value={weekDate} onChange={e => setWeekDate(e.target.value)} /></label>}
