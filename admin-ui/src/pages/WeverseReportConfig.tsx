...
export function WeverseReportConfig() {
  ...
  return <section className="wv-card">
    ...
    {settings && <>
      ...
      <label className="wv-check"><input type="checkbox" checked={settings.enabled} onChange={e => setSettings({ ...settings, enabled: e.target.checked })} />启用定期活动报告邮件</label>
      <div className="wv-fields">
        <label className="wv-check"><input type="checkbox" checked={settings.monthly} onChange={e => setSettings({ ...settings, monthly: e.target.checked })} />月报：次月 1 日</label>
        <label className="wv-check"><input type="checkbox" checked={settings.firstHalf} onChange={e => setSettings({ ...settings, firstHalf: e.target.checked })} />上半年报：7 月 1 日（1–6 月）</label>
        <label className="wv-check"><input type="checkbox" checked={settings.annual} onChange={e => setSettings({ ...settings, annual: e.target.checked })} />年报：次年 1 月 1 日</label>
        <label>报告发送时间（北京时间）<input type="time" value={settings.sendTime} onChange={e => setSettings({ ...settings, sendTime: e.target.value })} /></label>
      </div>
      ...
      <div className="inline-actions">
        <button disabled={!!busy} onClick={() => void action('preview', async () => { setReport(await api<Report>(`weverse/reports/preview?${query}`)) })}>预览统计</button>
        <a className="secondary-button" href={`/api/weverse/reports/download?${query}`}>下载 Excel</a>
        <button disabled={!!busy || data?.backfill.running} onClick={() => void action('backfill', async () => { await api(`weverse/reports/backfill?${query}`, { method: 'POST' }); setData(await api<Status>('weverse/reports')); setMessage('历史回采已启动，原始记录会逐步补齐，不补发旧动态') })}>回采当期历史</button>
        <button disabled={!!busy || !data?.emailEnabled} onClick={() => void action('send', async () => { await api(`weverse/reports/send?${query}`, { method: 'POST' }); setMessage(`报表和附件已发送至 ${data?.emailTo}`) })}>{busy === 'send' ? '发送中…' : '发送至邮箱'}</button>
      </div>
